...
module.exports.attachPaginateRestList = function attachPaginateRestList() {
    Knex.QueryBuilder.extend(
        ...
        function paginate({
            ...
            const { pagination: rawPagination, sort, filters } = formatQueryParameters(queryParameters);
            ...
            if (debug) {
                ...
                signale.debug('Formated query parameters', formatQueryParameters(queryParameters));
                ...
                filtersParameters.map((filter) => {
                    switch (filter.operator) {
                        ...
                        case FILTER_OPERATOR_NIN:
                            ...
                            break;
                        default:
                            debug && signale.log(`The filter operator ${filter.operator} is not managed`);
                    }
                });
            }
            ...
            if (shouldFetchTotals) {
                countQuery = new this.constructor(this.client)
                    .count('* as total')
                    .from(this.clone().offset(0).clearOrder().as('__count__query__'))
                    .first()
                    .debug(this._debug);
            }
