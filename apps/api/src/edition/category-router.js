...
const { deleteOne, createOne, getOne, getPaginatedList, updateOne } = require('./category-repository');
...
router.get('/:categoryId', async (ctx) => {
    ...
    if (!category.id) {
        const explainedError = new Error(`The category of id ${ctx.params.categoryId} does not exist.`);
...
router.delete('/:categoryId', async (ctx) => {
    ...
    if (!deletedCategory.id) {
        const explainedError = new Error(`The talk types of id ${ctx.params.categoryId} does not exist.`);
        explainedError.status = 404;
...
router.put('/:categoryId', async (ctx) => {
    const updatedCategory = await updateOne(ctx.params.categoryId, ctx.request.body);
